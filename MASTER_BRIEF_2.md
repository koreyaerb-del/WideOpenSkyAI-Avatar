...
> ▶ **NEXT SESSION — START HERE (paused 2026-07-31 · 🎉 LAYER 2 COMPLETE — full avatar + dashboard LIVE & PROVEN end-to-end):** The complete Live AI Avatar + Performance Dashboard now works end-to-end on WOSA's OWN account (dogfood). A visitor opens the SDK avatar page (`/avatar-live.html`), optionally enters name/email, talks to **Jenny by voice OR text**, and on End the transcript is captured → **Claude (`claude-haiku-4-5`) writes a narrative AI recap + extracts name/email/phone + classifies outcome (Booked/Captured/Browsed) + lists the real questions asked** → the rebuilt dashboard shows KPI tiles (Avatar Visitors / Conversations / Contacts Captured / Consults Booked), a conversion funnel, and rich per-session cards. Verified live: a real test convo produced a "CAPTURED · Korey Erb" card with email+phone (phone was AI-extracted from speech, not typed) and all 4 questions. **NEXT SESSION FOCUS = two dashboard UX tweaks Korey requested: (1) a "Run / Print Report" export (PDF + CSV) button; (2) a date-range selector (Today / 7 / 14 / 30 / 60 / 90 days / YTD) that filters the KPIs, funnel, and session list. THEN: the per-client configurable template (`/<client>` pages from config), per-user auth + RLS on the dashboard, and email/HubSpot push when a new contact is captured.** History markers below. ▶ **PRIOR (paused 2026-07-29):** All marketing/front-end pieces are DONE and live (avatar page v2, showcase, homepage banner → liveavatarinfo). **Next focus = build the DATA FLOW for the Live AI Avatar performance dashboard, wired to email + HubSpot — on WOSA's OWN account first ("eat our own dog food"), so every client demo shows a real working system with real data, not a mockup.** Plan: **(1)** run the LiveAvatar postMessage capture test to lock how session data is ingested; **(2)** Korey creates a **Supabase** account + project, then Claude supplies the DB schema + the avatar-page instrumentation snippet; **(3)** build **Make.com scenarios** that fetch each session transcript, summarize it with an LLM, and write to Supabase → push an **email/Gmail** alert (founder notification) and a **HubSpot** contact/deal record; **(4)** point the white-label dashboard at the live Supabase data. Dogfooding = the WOSA instance IS the demo instance shown to prospects. Full build plan in Section 6 ("Dashboard data pipeline"). **Also now locked (2026-07-30): the productized "Live Avatar Package" (3 client deliverables) + the KEY per-client page/dashboard TEMPLATE requirement (config-driven so we never hand-build each client) — see the "Productized Client Offering" section.** **▶ BUILD PROGRESS (2026-07-31): Supabase project `wosa-live-avatar` is LIVE (West US/Oregon, free tier); dashboard schema created & verified — tables `clients`/`sessions`/`questions`/`events` + indexes + RLS enabled, WOSA seeded as first client (slug `wosa`). Capture method DECIDED: the hosted copy-paste embed exposes NO transcript API and NO session-end webhook, so we run a CUSTOM LiveAvatar web-SDK integration (`@heygen/liveavatar-web-sdk`) on a page we control — session token minted server-side so the API key never touches the browser. Stack = Supabase (DB/auth) + Vercel (hosts the avatar page + token function) + GitHub (repo). Korey chose "build the full custom avatar page now." NEXT = Phase 4: write the custom avatar page + secure token endpoint, deploy on Vercel, capture conversations+events into Supabase; then Phase 5 connect the dashboard, Phase 6 add per-user auth+RLS, Phase 7 dogfood test. ✅ **LAYER 1 LIVE & PROVEN (2026-07-31):** Vercel avatar app + `/api/ingest` → Supabase `events`; `page_view`/`chat_started` confirmed landing (dogfood funnel pipeline works). NEXT NOW = build the **configurable per-client template** (surrounding images/links/callouts via per-client config) + give it a proper URL, then Layer 2 (conversation-transcript capture). See latest Changelog entry.** Reminder: NEVER ask Korey to share/screenshot API keys.
...
- **▶ NEXT-UP DASHBOARD TWEAKS (Korey, 2026-07-31):** (1) **"Run / Print Report"** button on the dashboard → export current view as **PDF + CSV**. (2) **Date-range selector** (Today / 7 / 14 / 30 / 60 / 90 days / YTD) that filters KPIs, funnel, and the session list (add a `range` param to `/api/stats` + date filtering on events/sessions; add the toggle UI to `dashboard.html`). These are the first things to build next session.
- Canonical avatar subdomain: `liveavatar.` vs `avatar.` wideopenskyai.com?
- Give the Vercel app a proper custom domain (currently temp `wosa-avatar-app-jlnh.vercel.app`) — e.g. `liveavatar.wideopenskyai.com/<client>` — and decide how WOSA's tracked SDK page (`/avatar-live.html`) relates to the pretty GitHub-Pages marketing page. Promote `avatar-live.html` → `index.html` on the app once fully happy.
- Older sessions (pre-AI) show as plain "browsed" cards with basic summaries — cosmetic only; all NEW conversations get the full AI treatment.
...
- **⏳ TODO — Wix contact-form email routing (started 2026-07-30, PAUSED mid-task):** Goal = route the WOSA homepage contact-form notifications from Korey's **personal Gmail → `Hello@WideOpenSkyAI.com`**. **Blocker found:** the built-in Wix form-submission automation ("New submission received…") is a *transactional/app automation* whose recipient is **locked to "Collaborator Roles"** (currently Korey, via his Gmail login) — its Recipients → Edit will NOT accept an arbitrary email. **Solution path:** (1) add `Hello@WideOpenSkyAI.com` as a **Contact** (Customers & Leads → Contacts → + New Contact) — required, since non-collaborators must be contacts to receive; (2) on that automation click **••• → Duplicate** to create an editable **custom** automation; (3) in the copy: **Send an email → Recipients → Edit → "Specific contacts" → select `Hello@`**, then Apply/Save/**Publish Changes**; (4) **turn OFF the original** app automation so it stops delivering only to Gmail; (5) **test:** submit a sample entry on the live site, confirm it arrives at `Hello@` (check spam — sender is `notifications@wix-forms.com`) and appears under **Customers & Leads → Forms & Submissions**. **VERIFY FIRST:** multiple forms exist on the site (seen: "WideOpenSkyAI Inquiry Form", "WOSAI Contact Form", "AI Consulting Inquiry Form") — confirm **which form is actually embedded on the live homepage** (click it in the Editor to read its name) before editing, so we fix the right one. Claude offered to do the whole thing via **browser automation (Claude-in-Chrome)** next session — Korey just needs his Wix tab open and to say "go."
...
## Changelog
- **2026-07-31 (🎉 LAYER 2 COMPLETE — live conversation capture + AI recaps + rebuilt dashboard, PROVEN end-to-end)** — Built and shipped the whole Layer 2 stack on the `wosa-avatar-app` Vercel project (temp URL `wosa-avatar-app-jlnh.vercel.app`). **What now works:** visitor opens `/avatar-live.html` (LiveAvatar **web SDK** `@heygen/liveavatar-web-sdk` via `https://esm.sh/...`, not the sealed iframe), optional **pre-chat name/email form**, clicks Start → browser mic prompt → talks to **Jenny by voice, with a text box + mic-mute toggle as alternatives** → SDK events `user.transcription`/`avatar.transcription` build the transcript, `session.stopped`/`disconnected` + `pagehide` save it. On End, `/api/session` calls **Claude `claude-haiku-4-5`** (Anthropic Messages API) to produce a JSON recap: narrative `summary`, `outcome` (booked/captured/browsed), extracted `contact_name/email/phone`, and clean `questions[]` — form values win, AI fills gaps; falls back to a basic summary if the key is missing. Rebuilt `dashboard.html` = 4 KPI tiles (Avatar Visitors / Conversations / Contacts Captured / Consults Booked) + a conversion funnel + rich per-session cards (outcome badge, name/contact, date+duration, AI recap paragraph, "Questions asked" list); `/api/stats` now returns those fields + questions grouped by session + funnel KPIs. **PROVEN:** a live test produced a "CAPTURED · Korey Erb · korey@axceptmedia.com · 952-484-8059" card — phone was **AI-extracted from the spoken transcript** (not typed) — with a strong recap and all 4 questions listed. **Files (all in `wosa-avatar-app` repo):** `api/token.js` (mints LiveAvatar session token server-side; normalizes language word→code e.g. "English"→"en"), `api/session.js` (AI recap + save), `api/stats.js` (dashboard data), `api/ingest.js` (Layer-1 funnel events, unchanged), `avatar-live.html` (SDK page), `dashboard.html`, `index.html` (old Layer-1 page, untouched). **Vercel env vars now set:** `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `LIVEAVATAR_API_KEY` (rotated — old key ending `…262b` was revoked, new "Vercel Dashboard" key ending `…b384` in use), `AVATAR_ID` `3f291b22-0267-4fb6-a25b-847fb63604b0`, `VOICE_ID` `de5574fc-009e-4a01-a881-9919ef8f5a0c` (Ann), `CONTEXT_ID` `c8c22e67-5698-4302-96c3-c6ea603b4371` (Jenny), `LANGUAGE` `English`, `ANTHROPIC_API_KEY` (Korey's, no-expiry). **Debugging notes for the record:** the "Errors validating session token" wall was diagnosed with a temporary `/api/token-debug` endpoint (since deleted) that decoded the JWT + called `/v1/sessions/start` server-side — it revealed VOICE_ID had been pasted with the CONTEXT_ID value ("Voice not found"); fixing VOICE_ID → 201 success. Also: mic didn't engage until `new LiveAvatarSession(token, { voiceChat: true })` was added (SDK auto-requests mic on start). LiveAvatar token API confirmed: `POST https://api.liveavatar.com/v1/sessions/token`, header `X-API-KEY`, body `{mode:"FULL", avatar_id, avatar_persona:{voice_id, context_id, language}, is_sandbox:false}`, token at `data.session_token`; SDK constructor takes the raw token string, `start()` calls `/v1/sessions/start` with `Authorization: Bearer <token>`. **Workflow note:** Korey commits files via GitHub web (create/paste), Vercel auto-deploys from `main`; `.js` downloads failed for him so paste-into-GitHub is the reliable method; give **one clean code block per file** with exact filename + overwrite steps (his stated preference). Claude can self-verify deploys via WebFetch on the live URLs (did so this session). No dedicated GitHub-commit connector exists in the registry; a **Vercel connector** (read-only: projects/deployments/logs) is available if Korey wants Claude to read deploy status directly. **NEXT:** the two dashboard tweaks in §9 (report export + date-range selector), then per-client template, dashboard auth+RLS, and email/HubSpot push.
- **2026-07-31 (Layer 1 LIVE — dogfood pipeline PROVEN)** — Built & deployed the custom avatar app (`wosa-avatar-app` repo) on **Vercel** (temp URL `wosa-avatar-app-jlnh.vercel.app`): a WOSA-branded avatar landing page (start-gate → LiveAvatar embed) + a `/api/ingest` serverless function (holds the Supabase secret key server-side) + anonymous funnel tracking (visitor cookie, `page_view`, `chat_started`). **CONFIRMED WORKING:** real `page_view` + `chat_started` rows landing in Supabase `events` from the live app. Vercel env vars = `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` (new-style `sb_secret_` key). **Important framing:** this Vercel app = WOSA's OWN white-label instance (client #1 / dogfood), currently at a temp `.vercel.app` URL + a simple Layer-1 layout; the marketing page at `liveavatar.wideopenskyai.com` (GitHub Pages) is prettier but does NOT capture data. Mid-build hiccup fixed: app files were first uploaded to the WRONG repo (`WideOpenSkyAI-Avatar`), overwriting the live `index.html` — restored from backup. **NEW REQUIREMENT (Korey):** the white-label avatar landing page must be **per-client CONFIGURABLE** — an easy way for Korey/client to add images, links, and product/service callouts *around* the avatar via a per-client config (content "blocks"), no hand-coding. **NEXT:** (a) build the configurable per-client template (config-driven blocks + branding by slug, rendering the surrounding content); (b) give it a proper address (`liveavatar.wideopenskyai.com/<client>`) and decide how WOSA's own tracked page relates to the current marketing page; (c) Layer 2 = conversation-transcript capture via the LiveAvatar web SDK (`@heygen/liveavatar-web-sdk`); then dashboard UI on live data + auth.
- **2026-07-31 (dashboard build — Phases 1-3 done)** — Kicked off the live performance-dashboard build (dogfooded on WOSA's own account). DONE: (1) created Supabase project `wosa-live-avatar` (West US/Oregon, free); (2) ran the schema — tables `clients`/`sessions`/`questions`/`events` + indexes + RLS enabled + WOSA seeded (verified via `information_schema`; Table Editor was blank only due to a Supabase platform incident, DB is fine); (3) DECIDED the capture method — the hosted copy-paste embed has NO transcript endpoint and NO session-end webhook, so we'll build a CUSTOM LiveAvatar web-SDK integration (`@heygen/liveavatar-web-sdk`) on a page we control, token minted server-side. Stack = Supabase + Vercel + GitHub. Korey chose "build the full custom avatar page now." Schema file: `wosa_dashboard_schema.sql`. NEXT = Phase 4 (write + deploy the custom avatar page). Full phase list in the task tracker.
- **2026-07-30 (Wix form routing — diagnosed, paused)** — Walked Korey through changing the homepage contact-form notification recipient (personal Gmail → `Hello@WideOpenSkyAI.com`). Found the built-in Wix form automation is transactional/app-locked to "Collaborator Roles" — can't set an arbitrary email. Logged the full duplicate-into-custom-automation fix + the "verify which of the 3 forms is live on the homepage" caution as a pending TODO in §9. Paused for a break; to resume, Claude can do it via browser automation.
